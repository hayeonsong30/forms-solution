...
import { saveExcelTemplateSchema } from "@/lib/schemas";
...
// 양식 버전당 활성 Excel 템플릿 1개.
export async function GET(_req: Request, ctx: RouteContext<"/api/template-versions/[versionId]/excel-template">) {
...
export async function DELETE(_req: Request, ctx: RouteContext<"/api/template-versions/[versionId]/excel-template">) {
  const { versionId } = await ctx.params;
  await prisma.excelReportTemplate.deleteMany({ where: { templateVersionId: versionId } });
